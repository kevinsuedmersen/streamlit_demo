...
from src.clients._base_api_client import BaseAPIClient
...
class BackendAPIClient(BaseAPIClient):
    def __init__(self, settings: Settings, api_token_cls: BaseAPIToken) -> None:
        self._settings = settings 
        self._api_token_cls = api_token_cls
    ...
    def _get_api_token(self) -> BaseAPIToken:
        if (self._backend_api_token is None) or self._backend_api_token.is_expired:
            logger.debug("Generating new api token.")
            self._backend_api_token = self._api_token_cls.from_signed_in_user(self._settings)
